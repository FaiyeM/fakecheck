...
| F9b | Ownership binding (scan/image ↔ device/user) | §9 / §1.1 | P0 | 🔄 | F0 (scan_photos col) | **Device-ownership half implemented 2026-06-28 (awaiting Faye's local `dotnet test`).** New pure `FakeCheck.Core/Security/ScanOwnership.cs` (`OwnershipResult` enum + `Check(requestDeviceId, scanOwnerDeviceId)`) + thin wiring in `AuthController.Analyze` and `CorrectionsController.Submit`: load the scan, compare the already-sent `X-Device-Id` header to `scan.DeviceId` → `401` missing / `404` no scan / `403` mismatch, else proceed. **No mobile change** (client already sends `X-Device-Id` via axios interceptor) and **no migration**. 9 new xUnit tests. **Remaining (needs F9h):** reject image keys not linked to the scan — requires `scan_photos` persisted server-side. |
...
|---|---|---|---|---|
| 2026-06-28 | interactive; backend NOT run in sandbox (no .NET SDK) — **Faye to run `dotnet build/test` locally**. F9a confirmed CI-green before starting. | F9b (device-ownership slice) | Implemented the ownership gate on analyze + corrections. New pure `ScanOwnership` kernel (4-state result) + thin controller wiring comparing the already-sent `X-Device-Id` header to `scan.DeviceId`. No mobile change, no migration. 9 new xUnit tests. Image-key-linking part deferred (needs F9h scan_photos). F9b → 🔄 pending Faye's local green + commit. | (pending Faye local verify) |
