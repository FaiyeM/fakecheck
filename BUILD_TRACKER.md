...
| F9a | Parallelize vision calls in `/auth/analyze` | §9 | P0 | 🔄 | — | **Implemented 2026-06-28 (awaiting Faye's local `dotnet test`).** New pure `FakeCheck.Core/Concurrency/BoundedParallel.cs` (`SemaphoreSlim`-bounded `Task.WhenAll`, order-preserving) + `VisionOptions.MaxParallelAuthCalls` (default 4). `AuthController.Analyze` now fans out the per-photo premium calls instead of the sequential `foreach`; results stay in photo order so persisted checks + DTOs are deterministic. 4 new xUnit tests (`BoundedParallelTests`: caps concurrency, real parallelism, order preserved, empty/serial). Persist still sequential (no concurrent DbContext). |
