...
| 8 | Backend deploy (Railway) | 🔄 config done, deploy = Faye runs CLI | `railway.json` (Dockerfile builder + `/health` healthcheck) added; `DATABASE_URL` auto-converted to Npgsql in DependencyInjection.cs so Railway managed PG works with zero manual config; `docs/DEPLOY_RAILWAY.md` runbook written. Actual `railway up` must be run by Faye (CLI + network absent in sandbox). |
